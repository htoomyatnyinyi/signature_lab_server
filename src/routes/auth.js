...
// Function to generate the username from email
...
router.post("/signup", (req, res) => {
  ...
  sql_db.query(checkUserQuery, checkUserParams, (err, results) => {
    ...
    if (identifier.includes("@mlab.com")) {
      generatedUsername = getUsernameFromEmail(identifier); // Use email as username
    } else if (/^\d+$/.test(identifier) && identifier.length > 7) {
      generatedUsername = generateUsername(); // Generate username for phone numbers
    } else {
    ...
    const insertUserQuery =
      "INSERT INTO users (email, phone, password, userType, username) VALUES (?, ?, ?, ?, ?)";
    const insertUserParams = [
      identifier.includes("@mlab.com") ? identifier : null,
      /^\d+$/.test(identifier) && identifier.length > 7 ? identifier : null,
      hashedPassword,
      userType,
      generatedUsername,
    ];
